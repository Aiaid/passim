...
#
# Environment variables (invite mode — auto-join an existing Hub):
#   INVITE=<token>      Invite token generated by the Hub
#   HUB=<https-url>     Hub address, e.g. https://abc.dns.passim.io:8443
#   NODE_NAME=<name>    Optional display name for this node
#   NODE_ADDR=<addr>    Optional override for this node's reachable address
#
#   When INVITE and HUB are both set, this script skips the API key prompt and
#   the new container will register itself with the Hub on first boot.
# ──────────────────────────────────────────────────────────────
set -euo pipefail
...
# ── Invite-mode env (passed straight through to the container) ────
# When INVITE+HUB are both set, the new node will auto-register with the Hub.
INVITE="${INVITE:-}"
HUB="${HUB:-}"
NODE_NAME="${NODE_NAME:-}"
NODE_ADDR="${NODE_ADDR:-}"
INVITE_MODE=false
if [[ -n "$INVITE" && -n "$HUB" ]]; then
  INVITE_MODE=true
  # Export so `docker run -e INVITE -e HUB` (no value) inherits from our env.
  # This keeps the token out of the rendered argv string.
  export INVITE HUB
  [[ -n "$NODE_NAME" ]] && export NODE_NAME
  [[ -n "$NODE_ADDR" ]] && export NODE_ADDR
fi
...
# Invite-mode envs: use `-e VAR` (no value) so docker inherits from this shell's
# environment — keeps the token out of the rendered argv / process listing / logs.
if $INVITE_MODE; then
  DOCKER_ARGS+=(-e INVITE -e HUB)
  [[ -n "$NODE_NAME" ]] && DOCKER_ARGS+=(-e NODE_NAME)
  [[ -n "$NODE_ADDR" ]] && DOCKER_ARGS+=(-e NODE_ADDR)
fi
...
if $INVITE_MODE; then
  echo -e "  ${CYAN}Hub:${NC}      ${HUB}"
  echo -e "  ${CYAN}Status:${NC}   joining hub — node will appear in the Hub's list within ~10 minutes"
else
...
echo -e "    docker logs -f passim        # View logs (watch for 'hub join' messages)"
