...
同时支持一键开通云存储 (S3 兼容)，用户不需要知道"对象存储"是什么。
...
**优先级:**
- 第一批: Vultr / DigitalOcean / Hetzner / AWS Lightsail / Linode (API 简单、价格实惠、个人用户主流)
- 第二批: AWS EC2 / GCP / Azure (API 复杂，后续加)
- 存储专项: Cloudflare R2 (无 VPS，只作为存储选项，免出站费)
...
- [ ] 设置 → 云账号管理，可添加多个云厂商账号
- [ ] 支持的厂商及认证方式:
  | 厂商 | 认证 | API Key 获取方式说明 |
  |------|------|---------------------|
  | Vultr | `Bearer` Token | 链接到 Vultr API 设置页 |
  | DigitalOcean | `Bearer` Token | 链接到 DO API Token 页 |
  | Hetzner Cloud | `Bearer` Token | 链接到 Hetzner API Token 页 (per-project) |
  | AWS Lightsail | Access Key + Secret Key | 链接到 AWS IAM 页 + 说明只需 Lightsail 权限 |
  | Linode (Akamai) | `Bearer` Token (PAT) | 链接到 Linode API Token 页 |
  | Cloudflare (仅存储) | `Bearer` API Token | 链接到 CF API Token 页 + 说明只需 R2 权限 |
- [ ] 添加后立即验证 API Key 有效性 (调用账号信息接口)
- [ ] 验证成功显示: 账户余额 / 账户邮箱
- [ ] 验证失败显示人话错误: "API Key 无效" / "权限不足，需要 xxx 权限"
...
## US-11.6 云服务向导 (新手全引导) `P1` `Phase 5+`
...
interface CloudProvider {
  ...
  supportsVps: boolean;               // true = VPS 厂商, false = 仅存储

  // 账号验证
  validateCredentials(creds: ProviderCredentials): Promise<AccountInfo>;

  // VPS 管理 (仅 supportsVps == true 时可用)
...
interface ProviderCredentials {
  ...
  region?: string;                   // Lightsail 需要指定区域
...
interface Plan {
  ...
  hourlyPrice?: number;
  recommended: boolean;              // 适合 Passim 的最便宜套餐
}

interface Instance {
  id: string;                        // UUID / Int / Name (厂商不同)
  ip?: string;
  ipv6?: string;
  ...
  plan: string;
  createdAt: string;
  tags?: Record<string, string>;     // 统一为 key-value，适配层负责转换
}
```
